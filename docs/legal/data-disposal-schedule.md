...
| Provider tokens and keys (Plaid, Coinbase, YNAB, Discogs, TrueLayer, Kraken, Kalshi, Alpaca) | While the connection exists | Row deleted at disconnect, Plaid included since the unlink route deletes `plaid_items`. Nothing retains a revoked credential, with the single bounded exception below |
| Plaid access token pending removal (`plaid_removal_queue`) | Only until Plaid accepts `/item/remove`, retried on the scheduler tick | **The one exception to the row above, and it exists for the user's benefit, not ours.** Both unlink paths deliberately do not block on Plaid being reachable, and both used to destroy the item row (the only copy of the access token) even when `/item/remove` failed. Plaid bills a subscription Item monthly "even if no API calls are made for the Item", ending it requires that call, and a Trial Item that is removed does not return its connection slot. So a swallowed failure left an Item billed forever that nobody could cancel. The queue row holds an item id and the encrypted token and **no `user_id` and no foreign key**: it is not joinable to a person, it is not read by any product path, and `dequeueItemRemoval` deletes it the moment the removal succeeds or Plaid reports the Item already gone (`ITEM_NOT_FOUND`, `INVALID_ACCESS_TOKEN`). Registered in `db/rotate-encryption-key.ts` so a key rotation cannot strand it |
...
| Operational events (`ops_events`) | 90 days | Purge job (built). **Contains no personal data by construction**: the table has no user column, and rows carry a vendor hostname, an asset class, a task name and a programmatic error class only. It is listed here for completeness of the disposal record rather than because it holds customer information. Ninety days answers the longest question anyone asks of it ("has this vendor been flaky all quarter") and bounds a failure loop that would otherwise grow the table without limit |
