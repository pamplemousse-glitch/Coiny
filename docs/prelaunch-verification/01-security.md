...
| 1.8.6 | Nothing sensitive reaches a crash report | Check what crash pipeline exists | NOT APPLICABLE today | No crash reporting exists in either client (`docs/launch-gap-analysis.md` §1 item 7 verified no `MetricKit` import); nothing can leak through a pipeline that does not exist. It becomes a live question the moment MetricKit payloads are posted to `/api/telemetry`, and the payloads are Apple-shaped diagnostics, not app state |
...
| 1.11.9 | The support address that a disclosure would arrive at actually exists | Check the address | UNVERIFIED | PRD §29 records `coiny@athanorworks.com` as Unverified, domain and alias unconfirmed. Settles with the same decision as 1.11.8 |
