...
| DR-33 | 2026-08-19 | **App Store availability is restricted to the United States at launch.** Not merely "we market to the US": the store record itself is US-only. | Founder decision, extending DR-30 from a marketing posture to an enforced one. Three reasons: GDPR binds the moment an EU resident uses the app regardless of a US LLC, so restricting drops that exposure to zero on day one; the product is materially broken outside the US, since Spinwheel is pinned to VantageScore/Equifax and a non-US user therefore gets no debts and no credit score, making net worth assets-only and wrong in the flattering direction; and availability is reversible at any time in App Store Connect, so this costs a setting rather than a commitment. **Known cost, accepted:** it makes international demand invisible, so the waitlist country field in `expansion-plan.md` Phase 1 is the compensating instrument | Founder action in App Store Connect at G3.1. Sequencing for lifting it is `docs/expansion-plan.md` |
...
| DR-32 | 2026-08-13 | **Retirement contribution data is reachable through Plaid after all.** `/investments/holdings/get` returns `vested_quantity` and `vested_value` per holding, and `/investments/transactions/get` carries a `contribution` subtype, so contribution rate is computable as contributions over observed paycheck inflows for any connected account. DR-22's "no integration exposes vesting" is corrected: it is true of employer-plan recordkeepers, not of Plaid | Verified in the repo's own vendor docs at `docs/context/plaid.md:13445,13928`. The earlier conclusion came from a grep for "vest" whose every hit was the substring inside "investment", which buried the real fields | Unbuilt. `net-worth.ts` sums only `institution_value` and discards the vested fields; investments transactions are not called at all. Note the second Plaid subscription cost |
