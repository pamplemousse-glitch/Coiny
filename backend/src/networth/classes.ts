...
// additive for clients. `reauth_required` and `expiring` need the Plaid item
// lifecycle status column (R-8.5), which is a separate workstream; until it
// lands those statuses are never emitted here.
...
export function deriveStatus(input: StatusInput): ClassStatus {
  ...
  if (input.asOf === null) return 'stale';
