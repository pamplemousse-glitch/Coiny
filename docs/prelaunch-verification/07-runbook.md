...
| # | Do this | Who | Closes | Lead time, blocking |
|---|---|---|---|---|
| G1.1 | Check `developer.apple.com` > Membership for team `UKL98DS9D3`. If it reads Individual, start the Organization migration now; it needs the LLC's D-U-N-S number. | [Founder] | 5.5.5 | **Weeks, and it is the longest pole.** Blocks every archive, therefore TestFlight, therefore G1.16, G1.17, G3.2, G3.8, G3.10, and every device-dependent row in Part 4 (4.2.1, 4.4.1, 4.10.5, 4.10.6, 4.11.1) and Part 6 (6.1.14, 6.2.11, 6.4.6). Apple 5.1.1(ix) bars individual enrollments for financial services |
