...
extension API {
    ...
    func addManualAsset(name: String, category: String, value: Double, notes: String?) async throws {
        struct Body: Encodable { let name: String; let category: String; let selfReportedValueUsd: Double; let notes: String? }
        let _: EmptyResponse = try await post("/api/manual-assets", body: Body(name: name, category: category, selfReportedValueUsd: value, notes: notes))
    }
