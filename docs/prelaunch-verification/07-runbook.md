...
| G3.10 | ~~Add the MetricKit subscriber posting `MXMetricPayload` to the existing `/api/telemetry`...~~ **DONE 2026-08-23, both halves.** Metrics in #325, diagnostics after. **Correction to this row:** the payload CANNOT go to `/api/telemetry`. `analytics/events.ts` accepts only flat objects whose strings match a lowercase token regex and rejects unknown keys rather than stripping them, which is the control keeping merchant names out of analytics; a nested tree of histograms and a call stack tree cannot pass it. Metrics are REDUCED on device to a `device_metrics` catalog event, and diagnostics go to a new `POST /api/diagnostics`. The same wrong instruction appears in `04-performance-reliability.md` 4.13.3 and `launch-gap-analysis.md` §7. | [Founder] approved the labels, [Agent] built | 4.13.1, 4.13.2, 4.13.3, 1.8.6 | Done. It is a system framework, so it adds no vendor and does not contradict PRD §24. **Unblocks** 4.2.1, 4.4.1, 4.10.6 and the crash-free-sessions indicator |
